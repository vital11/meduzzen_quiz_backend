...
from app.db.dependencies import get_current_user, get_current_owner_user
...
async def read_company(
        id: int, current_user: User = Depends(get_current_user)) -> Company:
    company_repo = CompanyRepository(db=database, current_user=current_user)
...
async def read_companies(
        current_user: User = Depends(get_current_user)) -> list[Company]:
    company_repo = CompanyRepository(db=database, current_user=current_user)
...
async def update_company(
        id: int, payload: CompanyUpdate,
        current_user: User = Depends(get_current_owner_user)) -> Company:
    company_repo = CompanyRepository(db=database, current_user=current_user)
...
async def delete_company(
        id: int, current_user: User = Depends(get_current_owner_user)) -> Company:
    company_repo = CompanyRepository(db=database, current_user=current_user)
