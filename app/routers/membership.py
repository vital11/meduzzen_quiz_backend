...
from app.db.dependencies import get_current_user, get_current_active_superuser, get_current_owner_user
...
async def read_memberships_all() -> list[Membership]:
...
async def reject_membership(
        id: int, current_user: User = Depends(get_current_user)
) -> Membership:
    membership_repo = MembershipRepository(db=database, current_user=current_user)
...
@router.get('/members/companies/{company_id}',
            dependencies=[Depends(get_current_user)],
            response_model=list[CompanyMember])
async def read_company_members(company_id: int) -> list[CompanyMember]:
    membership_repo = MembershipRepository(db=database)
    return await membership_repo.get_company_members(company_id=company_id)


@router.get('/members/users/{user_id}',
            dependencies=[Depends(get_current_user)],
            response_model=list[MemberCompany])
async def read_member_companies(user_id: int) -> list[MemberCompany]:
    membership_repo = MembershipRepository(db=database)
